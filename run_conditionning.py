...
class StatisticalDataCollatorForLanguageModeling(DataCollatorMixin):
    ...
    def torch_mask_tokens(self, inputs: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        ...
        does_make_stats = CallbackForGradientStatistics._DOES_MAKE_STATS
        labels = inputs.clone()
        # We sample a few tokens in each sequence for MLM training (with probability `self.mlm_probability`)
...
compression_extension = '.gzip'
checkpoint_prefix = 'checkpoint-'

def get_all_checkpoints_file_names(logging_dir: str, checkpoint_prefix: str = 'checkpoint-') -> list[str]:
    return [file_name for file_name in os.listdir(logging_dir) if len(file_name) > len(checkpoint_prefix) and file_name[:len(checkpoint_prefix)] == checkpoint_prefix]

def parse_checkpoint_numbers(checkpoint_files_list: list[str], checkpoint_prefix: str = 'checkpoint-') -> list[int]:
    return [int(file_name[len(checkpoint_prefix):]) for file_name in checkpoint_files_list]

def get_checkpoints_filenames(logging_dir: str, filter_on_checkpoint_number: Callable[[int], bool], checkpoint_prefix: str = 'checkpoint-') -> list[str]:
    _numbers = parse_checkpoint_numbers(
        get_all_checkpoints_file_names(
            logging_dir,
    ...
    _numbers.sort()
    return [
        f'{checkpoint_prefix}{filtered_number}'
        for filtered_number in filter(
            filter_on_checkpoint_number,
            _numbers
        )
    ]

def get_checkpoints_paths(logging_dir: str, filter_on_checkpoint_number: Callable[[int], bool], checkpoint_prefix: str = 'checkpoint-') -> list[str]:
    return [
        os.path.join(logging_dir, filename)
        for filename in get_checkpoints_filenames(logging_dir, filter_on_checkpoint_number, checkpoint_prefix=checkpoint_prefix)
    ]


pairwise_checkpoints_paths = get_checkpoints_paths('model_output', lambda n: n%2 == 0)

pairwise_checkpoints_paths

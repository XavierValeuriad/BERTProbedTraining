            ...
            try:
                ...
                _SAVING_THREAD_POOL.submit(
                    ...
                    os.path.join(
                        ...
                        f'counter_{next(CallbackForGradientStatistics._ATOMIC_COUNTER)}#collarcounter_{StatisticalDataCollatorForLanguageModeling._ATOMIC_COUNTER}#epoch_{trainer.state.epoch}#step_{trainer.state}#device_{torch.cuda.current_device()}#time_{datetime.now().strftime("%I:%M%p on %B %d, %Y")}.json'.replace(' ', '_').replace(',', '_')
                    ),
                    gradient_statistics
                )
